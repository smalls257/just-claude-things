...
Emit a single file `migrations/0001_initial_schema.sql` containing every
SQL fence from every module's `<entities>` block, concatenated in module
order then entity order.
...
-- Source modules: Orders, Customers, Shared
...
- The `migrations/` folder lives at repo root by default. If
  `db/migrations/` already exists at scaffold time, use that instead.
- If a SQL fence inside `<entities>` references another table that
  doesn't exist yet in the concatenated output, **do not reorder.**
  Print a warning at the end of the SQL file:
  ```sql
  -- WARNING: foreign-key reference detected; verify table emission order matches your CREATE constraint expectations.
  ```
- FK CONSTRAINT clauses inside CREATE TABLE statements are preserved
  as-is — devs handle ordering issues by editing the tech-design.

### Folder placement

If `db/migrations/` exists at scaffold root, use it. Otherwise use
`migrations/`. Decision logged in summary report.
