...
**Row → Domain conversion:** the Persistence row type carries the raw `string` DB value for enum columns. The mapper that converts `{{ENTITY}}Row` to `{{ENTITY}}` uses `Enum.Parse<{{ENUM}}>(row.{{COLUMN}}, ignoreCase: true)`. The mapper lives in Persistence (authored by devs); Phase-2 does not generate it.
...
{
    [Fact]
    public void Placeholder() => throw new NotImplementedException("TODO: write {{ENTITY}} tests");
}
```
...
`{{PATH_SAFE}}` is the path with `/` → `_`, `{x}` → `byX`. E.g., `/orders/{id}` → `orders_byId`.
...
{
    [Fact]
    public void Placeholder() => throw new NotImplementedException("TODO: write {{METHOD}} {{PATH}} integration test");
}
```
